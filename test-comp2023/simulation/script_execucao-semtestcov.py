    ...
    try:
        with open(yml_path, "r") as file:
            ...
            if "input_files" in data:
                ...
                print("[INFO] Input file encontrado com properties/coverage-error-call.prp: {}".format(input_file_path))
...
def executar_ferramenta(comando, arquivo, output_dir, cwd=None):
    ...
    try:
        ...
        # Em caso de timeout, tenta capturar todo o output gerado (sem aplicar strip)
        timeout_output = e.output if e.output is not None else ""
        if not timeout_output:
            ...
            timeout_output += "UNKNOWN"
        print("[TIMEOUT] Processo excedeu 360 segundos: {}".format(arquivo))
        return 360.0, None, timeout_output
        
    except Exception as e:
        error_output = "Erro de execuçAo: {}".format(str(e))
        print("[ERRO CRITICO] {}".format(error_output))
        return (time.time() - inicio), None, error_output
...
def processar_subcategoria(subcategoria_path, destino, categoria, subcategoria_nome):
    ...
    for yml_file in arquivos_yml:
        ...
        for input_file in input_files:
            ...
            # Cria a pasta do programa (mesmo que o test_suite nAo seja gerado)
            ...
            try:
                tempo_execucao, test_suite_path, output = executar_ferramenta(
                    comando, input_file, destino, cwd=cwd_release
                )
            except Exception as e:
                print("[ERRO CRITICO] Falha na execuçAo: {}".format(e))
                tempo_execucao = 360.0
                test_suite_path = None
                output = "Erro de execuçAo: {}".format(str(e))
            
            # Se test_suite.zip foi gerado, move-o para a pasta do programa
            ...
            # Grava TODO o output gerado pelo Map2check no arquivo map2check_output.txt
            map2check_output_path = os.path.join(program_dir, "map2check_output.txt")
            with open(map2check_output_path, "w") as f_out:
                f_out.write(output + "\n")
            print("[INFO] Output completo gravado em: {}".format(map2check_output_path))
            
            # Extrai o veredito final: ultima linha nAo vazia do output
            verdict = "UNKNOWN"
            ...
            linha = "{}: {} segundos, Resultado: {}".format(input_filename, tempo_formatado, verdict)
